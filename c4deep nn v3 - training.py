...
plot_model(model, to_file='model_deep.png', show_shapes= True, show_layer_names=True)
...
try:
    model = load_model('connect_four_deep.h5')
    model_2 = load_model('connect_four_2_deep.h5')
    ...
    for game in games: #game is the full game history from all empty squares, to the finished game state. games is a list of all these game histories.
        ...
        for i in range(1, len(game)):
            if i % 2 == 1: #this shows if it was X's turn???
                for j in range(0, 42):
                    if not game[i-1][j] == game[i][j]: ##i is the gamestate history leading up to the final gamestate. j is the individual squares in the game histories. so this is comparing individual squares and seeing if they changed
                        reward_vector = np.zeros(7) # return an array of a given shape filled with zeros.
                        reward_vector[j%7] = total_reward * (reward_dep ** (math.floor((len(game) - (i-1)) / 2) - 1)) # this exponent returns values between 0 and 4, based on game length. if you're in the won gamestate, total reward == reward vector == 0 or 1 or -1
                ...
                for j in range(0, 42):
                    if not game[i-1][j] == game[i][j]: ##copy for O's(?)
                        ...
                        reward_vector[j%7] = -1 * total_reward * (reward_dep ** (math.floor((len(game) - (i-1)) / 2) - 1))
...
while True: #this is basically the main function inside a while true loop
    ...
    if mode == 'training':
        ...
        for i in range(0, total_games):
            ...
            while playing: #playing will remain true until the game ends. hence we stick in here.
                if nn_turn:
                    ...
                    else: #else take model prediction. Interesting as it's random 70% of the time. Explains why so many wins/losses in training.
        ...
        while running:
            ...
            for x in range(0, 42, 7):
                print(r_board[x], r_board[x+1], r_board[x+2], r_board[x+3], r_board[x+4], r_board[x+5], r_board[x+6])
